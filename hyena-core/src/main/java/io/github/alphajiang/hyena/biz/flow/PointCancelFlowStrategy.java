...
public class PointCancelFlowStrategy extends AbstractPointFlowStrategy {
    ...
    public void addFlow(PointUsage usage, PointPo point) {
        ...
        try {
            do {
                var recLogsRet = this.cancelPointLoop(usage.getType(), usage.getUid(), point, gap, usage.getNote());
...
//                "no enough available point!");
    ...
    private List<PointRecLogPo> cancelPointLoop(String type, String uid, PointPo point, long expected, String note) {
        ...
        for (PointRecPo rec : recList) {
            ...
            if (gap < 1L) {
                ...
                var retRec = this.pointRecDs.cancelPoint(type, rec, gap, note);


                var recLog = this.pointRecLogDs.addLogByRec(type, PointStatus.CANCEL,
